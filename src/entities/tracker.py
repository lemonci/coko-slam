""" This module includes the Tracker class. The majority of this functionality is taken from:
    https://github.com/VladimirYugay/Gaussian-SLAM/blob/main/src/entities/tracker.py
"""
...
from src.utils.tracking_utils import (compute_camera_opt_params,
...
class Tracker(object):
    ...
    def track(self, frame_id: int, gaussian_model: GaussianModel, prev_c2ws: np.ndarray) -> np.ndarray:
        """
        Updates the camera pose estimation for the current frame based on the provided image and depth,
        using either ground truth poses, constant speed assumption, or visual odometry.
        Args:
        ...
        if self.odometry_type == "odometer" and self.odometer.last_rgbd is None:
        ...
        if self.odometry_type == "gt":
            ...
            init_c2w = prev_c2ws[2, :] @ np.linalg.inv(prev_c2ws[1, :]) @ prev_c2ws[2, :]
        ...
        best_rel_w2c = np2torch(init_rel_w2c)
        ...
        if len(self.frame_color_loss) > 0 and (
            ...
            if self.odometry_type != "odometer":
                _, last_image, last_depth, _ = self.dataset[frame_id - 1]
                self.odometer.update_last_rgbd(last_image, last_depth)
                odometer_rel = self.odometer.estimate_rel_pose(image, depth)
        ...
        self.logger.log_tracking_iteration(
            frame_id, transformation_to_quaternion(np2torch(init_c2w)), gt_quat, gt_trans,
            total_loss, color_loss, depth_loss, -1, -1,
            print_output=True, print_wandb=True)
        ...
        for iter in range(num_iters):
            ...
            tracking_time += time.time() - start_time_iter
            ...
            with torch.no_grad():
                if total_loss.item() < current_min_loss:
                    current_min_loss = total_loss.item()
                    best_rel_w2c = torch.eye(4)
                    best_rel_w2c[:3, :3] = build_rotation(F.normalize(opt_cam_rot[None].clone().detach().cpu()))[0]
                    best_rel_w2c[:3, 3] = opt_cam_trans.clone().detach().cpu()
                ...
                if iter == num_iters - 1:
                    cur_w2c = torch.from_numpy(reference_w2c) @ best_rel_w2c
                else:
                ...
                if iter == num_iters - 1:
                    ...
                    self.logger.log_tracking_iteration(
                        frame_id, cur_cam, gt_quat, gt_trans, total_loss, color_loss, depth_loss, iter, num_iters,
                        print_output=True, print_wandb=False)
        print(f"Tracking took {tracking_time:.3f} seconds")
        final_c2w = torch.inverse(torch.from_numpy(reference_w2c) @ best_rel_w2c)
        final_c2w[-1, :] = torch.tensor([0., 0., 0., 1.], dtype=final_c2w.dtype, device=final_c2w.device)
        self.logger.log_tracking_iteration(
            frame_id, transformation_to_quaternion(final_c2w), gt_quat, gt_trans, total_loss,
            color_loss, depth_loss, iter, num_iters, print_output=True, print_wandb=True)
